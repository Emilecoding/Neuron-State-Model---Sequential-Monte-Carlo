...
def particle_filter(
    ...
    sigma_dyn=0.3,
    sigma_obs=5.0,
    save_particles=False  # NOUVEAU: option pour sauvegarder
):
    """
    ...
    particles[:, 0] = -65 + 5*np.random.randn(N)
    particles[:, 1:] = [0.05, 0.6, 0.32]

    weights = np.ones(N) / N
    V_est = np.zeros(T)
    
    # NOUVEAU: stockage des particules et poids si demandé
    if save_particles:
    ...
    for t in range(T):
        ...
        for i in range(N):
            particles[i] = hh_step(
                particles[i], I, dt, noise_std=sigma_dyn
            )

        # Weight update
        if not np.isnan(y[t]):
            likelihoods = np.exp(
                -0.5 * ((y[t] - particles[:,0])**2) / sigma_obs**2
            )
...
if __name__ == "__main__":
    ...
    # MODIFICATION IMPORTANTE: on récupère aussi les particules
    V_filt, particles, weights = particle_filter(
        ...
        sigma_dyn=0.3,
        sigma_obs=5.0,
        save_particles=True  # MODIFICATION ICI
    )
    ...
    plt.scatter(t[~np.isnan(y_obs)],
                ...
                s=10, color="red", alpha=0.3, label="Observations")
    plt.legend()
    plt.xlabel("Time (ms)")
