...
from smoothing import backward_smoothing_corrected, smooth_expectation
from maximisation import (compute_sigma_obs, compute_sigma_dyn, 
                                  estimate_hh_parameters)

def em_algorithm_hh(y_obs, dt, n_iter=5, N_particles=200, initial_params=None):
    """
    ...
    for iteration in range(n_iter):
        ...
        smoothed_particles_list, smoothed_weights_list = backward_smoothing_corrected(
        ...
        sigma_dyn_new = compute_sigma_dyn(smoothed_particles, smoothed_weights, dt, params['I'])
        
        print(f"   ✓ sigma_obs: {sigma_obs_new:.3f} (précédent: {params['sigma_obs']:.3f})")
        print(f"   ✓ sigma_dyn: {sigma_dyn_new:.3f} (précédent: {params['sigma_dyn']:.3f})")
        ...
        for name, true in zip(param_names, true_values):
            ...
            print(f"   {name}: {old:6.2f} → {new:6.2f} (vrai: {true:6.2f}) "
        ...
        print(f"💾 Résultats sauvegardés: results/em_iterations/iteration_{iteration+1:02d}.npz")
...
def plot_em_convergence(history, true_params=None):
    ...
    for idx, (name, ax) in enumerate(zip(param_names, axes)):
        ...
        if true_params and name in true_params:
            ax.axhline(y=true_params[name], color='red', linestyle='--',
    ...
    axes2[0].axhline(y=5.0, color='red', linestyle='--', alpha=0.7, label='Vrai (5.0)')
    ...
    axes2[1].axhline(y=0.3, color='red', linestyle='--', alpha=0.7, label='Vrai (0.3)')
    ...
    if not np.all(np.isnan(history['rmse_smooth'])):
        ...
        ax3.plot(iterations, history['rmse_filt'], 'o-', linewidth=2,
                label='RMSE filtré', alpha=0.7)
        ax3.plot(iterations, history['rmse_smooth'], 's-', linewidth=2,
                label='RMSE lissé', alpha=0.9)
    ...
    if n_iter >= 2:
        ...
        try:
            ...
            axes4[0, 0].plot(history['V_smooth'][0][:500], 'b-', alpha=0.5, label='Itération 1')
            ...
            axes4[0, 1].plot(history['V_smooth'][-1][:500], 'r-', alpha=0.8, label=f'Itération {n_iter}')
            ...
            # Erreur
            axes4[1, 0].plot(np.abs(history['V_smooth'][0][:200] - V_true[:200]),
                           'b-', alpha=0.5, label='Itération 1')
            axes4[1, 0].plot(np.abs(history['V_smooth'][-1][:200] - V_true[:200]),
                           'r-', alpha=0.8, label=f'Itération {n_iter}')
            ...
            # Amélioration
            improvement = 100 * (1 - history['rmse_smooth'][-1] / history['rmse_smooth'][0])
            axes4[1, 1].text(0.1, 0.7, f'RMSE initial: {history["rmse_smooth"][0]:.3f} mV',
                           fontsize=12)
            axes4[1, 1].text(0.1, 0.5, f'RMSE final: {history["rmse_smooth"][-1]:.3f} mV',
                           fontsize=12)
            axes4[1, 1].text(0.1, 0.3, f'Amélioration: {improvement:.1f}%',
                           fontsize=14, fontweight='bold')
            axes4[1, 1].axis('off')
            
            plt.tight_layout()
            plt.savefig("figures/em/estimation_comparison.png", dpi=150, bbox_inches='tight')
            
        except:
            print("⚠  Impossible de créer les comparaisons (fichier hh_data.npz manquant)")
...
def run_em_from_scratch():
    ...
    n_iter = 3  # Seulement 3 itérations pour test
    N_particles = 100  # Peu de particules pour vitesse
    ...
    for name in param_names:
        ...
        print(f"{arrow} {name}: {estimated:6.2f} (vrai: {true:6.2f}) | erreur: {error_pct:5.1f}%")
