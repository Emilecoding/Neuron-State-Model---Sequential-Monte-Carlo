...
def compute_sigma_obs(smoothed_particles, y_obs, smoothed_weights):
    ...
    if np.sum(obs_mask) == 0:
        raise ValueError("y_obs doesn't have any point")
...
def compute_sigma_dyn(smoothed_particles, smoothed_weights, dt, I, sigma_dyn=0.2):
    """
    ...
    for t in range(1, T):
        for i in range(N):
            ...
            pred = hh_step(smoothed_particles[t - 1, i], I, dt, noise_std=sigma_dyn)
    ...
    sigma_dyn_new = np.sqrt(
        weighted_errors / total_weight / smoothed_particles.shape[2]
    )
