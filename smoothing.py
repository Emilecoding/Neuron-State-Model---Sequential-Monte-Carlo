...
def backward_smoothing_corrected(particles, weights, dt, I, sigma_dyn):
    """
    FFBS avec stabilité numérique
    
    Parameters
    ----------
    particles : array (T, N, state_dim) ou list
    weights : array (T, N) ou list
    """
    T = len(particles)
    N = particles[0].shape[0]
    
    # Convertir en listes si arrays numpy
    if isinstance(particles, np.ndarray):
        particles = [particles[t] for t in range(T)]
    if isinstance(weights, np.ndarray):
        weights = [weights[t] for t in range(T)]
    
    print(f"  Smoothing: T={T}, N={N}")
    
    # Étape backward
    beta = [np.ones(N) for _ in range(T)]  # β_T(i) = 1
    
    for t in range(T-2, -1, -1):
        if (t % 100 == 0):
            print(f"    Pas {t}/{T}", end='\r')
        
        # Calculer les log-transitions de manière vectorisée
        log_transitions = np.zeros((N, N))
        for i in range(N):
            # Prédiction déterministe
            pred = hh_step(particles[t][i], I, dt, noise_std=0.0)
            # Différence au carré (pour distribution gaussienne)
            diff = particles[t+1] - pred.reshape(1, -1)
            log_transitions[i] = -0.5 * np.sum(diff**2, axis=1) / sigma_dyn**2
        
        # Mise à jour backward avec logsumexp pour stabilité
        for i in range(N):
            log_beta_next = np.log(beta[t+1] + 1e-100)
            log_sum = logsumexp(log_transitions[i] + log_beta_next)
            beta[t][i] = np.exp(log_sum)
    
    print(f"    Backward pass terminé          ")
    
    # Calcul des poids lissés
    smoothed_weights = []
    for t in range(T):
        w_smooth = weights[t] * beta[t]
        w_smooth = w_smooth / (np.sum(w_smooth) + 1e-100)
        smoothed_weights.append(w_smooth)
    
    return particles, smoothed_weights
...
def plot_smoothing_results(V_true, V_filt, V_smooth, y_obs, dt, save_dir="figures"):
    ...
    axes[1].scatter(t[~np.isnan(y_obs) & mask_zoom], 
                   y_obs[~np.isnan(y_obs) & mask_zoom],
                   s=10, c='green', alpha=0.5, label='Observations')
    ...
    axes[2].plot(t[mask_obs][:300], error_filt[:300], 'b-', alpha=0.5, label='Erreur filtré')
    axes[2].plot(t[mask_obs][:300], error_smooth[:300], 'r-', alpha=0.8, label='Erreur lissé')
    axes[2].set_xlabel('Temps (ms)')
    axes[2].set_ylabel('Erreur absolue (mV)')
    axes[2].set_title(f'Erreurs (RMSE filtré: {np.mean(error_filt):.2f} mV, '
...
if __name__ == "__main__":
    ...
    smoothed_particles, smoothed_weights = backward_smoothing_corrected(
